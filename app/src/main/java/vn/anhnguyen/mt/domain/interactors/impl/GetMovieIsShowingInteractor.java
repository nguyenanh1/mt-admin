...
public class GetMovieIsShowingInteractor extends AbstractInteractor implements IGetMovieIsShowingInteractor {
    private IGetMovieIsShowingInteractor.Callback mCallback;
    ...
    public GetMovieIsShowingInteractor(Executor threadExecutor, MainThread mainThread,
                                       IGetMovieIsShowingInteractor.Callback mCallback, IAPIService mAPIService, IDeviceUtils mDeviceUtils,
    ...
    public void run() {
        if(mDeviceUtils.hasInternetConnection()) {
            ...
            ResponseGetMovie response = mAPIService.getIsShowingMovie(partdate,start,limit);
            if(response!=null){
    ...
    private void notifySuccess(final List<MovieCategory> movie) {
        mMainThread.post(new Runnable() {
            @Override
            public void run() {
                mCallback.getMovieIsShowingSuccess(movie);
            }
        });
    }
