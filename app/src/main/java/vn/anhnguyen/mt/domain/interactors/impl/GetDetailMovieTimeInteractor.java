...
public class GetDetailMovieTimeInteractor extends AbstractInteractor implements IGetDetailMovieTimeInteractor {
    private IGetDetailMovieTimeInteractor.Callback mCallback;
    private IAPIService mAPIService;
    private IDeviceUtils mDeviceUtils;
    private ISharedPrefUtils mSharedPrefUtils;
    private int idMovieTime;

    public GetDetailMovieTimeInteractor(Executor threadExecutor, MainThread mainThread, Callback mCallback, IAPIService mAPIService,
                                        IDeviceUtils mDeviceUtils, ISharedPrefUtils mSharedPrefUtils, int idMovieTime) {
    ...
    public void run() {
        if(mDeviceUtils.hasInternetConnection()){
            ...
            ResponseGetDetaiLMovieTime response = mAPIService.getDetailMovieTime(token,idMovieTime);
            if(response!=null){
                switch (response.getCode()){
                    ...
                    case CommonVls.LIST_EMPTY:
                        notifySuccess(new ArrayList<TicketDetail>());
                        break;
    ...
    private void notifySuccess(final List<TicketDetail> list) {
        mMainThread.post(new Runnable() {
            @Override
            public void run() {
                mCallback.getDetailMovieTimeSuccwss(list);
            }
        });
    }
